...
func NewTask(filePath string, url string, aliKey string) (*Downloader, error) {
	result, err := parse.FromURL(url, aliKey)
	if err != nil {
		return nil, err
	}
	var folder = path.Dir(filePath)
...
func (d *Downloader) mergeHsToMp4() error {
	...
	mFile, err := os.Create(path.Join(d.folder, d.fileName))
	if err != nil {
