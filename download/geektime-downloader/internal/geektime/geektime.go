...
func (c *Client) Login(phone, password string) ([]*http.Cookie, error) {
	...
	resp, err := r.Execute(r.r.Method, r.r.URL)
...
func (c *Client) Auth(cs []*http.Cookie) error {
	...
	r.r.SetHeader(Origin, DefaultBaseURL)
	resp, err := r.Execute(r.r.Method, r.r.URL)
...
func (c *Client) ProductInfo(productID int) (response.V3ProductInfoResponse, error) {
	...
	//fmt.Println(c.BaseURL + V3ProductInfoPath)
	r := c.NewRequest(resty.MethodPost,
		V3ProductInfoPath,
		nil,
		map[string]interface{}{
...
func (r *MyReq) Execute(method, url string) (*resty.Response, error) {
	x := rand.Intn(2000)
	time.Sleep(time.Duration(x) * time.Millisecond)
	//fmt.Println(method, url, r.r.Body)
	return r.r.Execute(method, url)
}

func (c *Client) NewRequest(method, url string, params map[string]string, body interface{}, res interface{}) *MyReq {
	r := c.HTTPClient.R()
	r.SetDebug(true)
	r.Method = method
	r.URL = c.BaseURL + url
...
func Do(r *MyReq) (*resty.Response, error) {
	resp, err := r.Execute(r.r.Method, r.r.URL)
	...
	return nil, ErrGeekTimeAPIBadCode{r.r.URL, resp.String()}
}
